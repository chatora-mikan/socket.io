...
Manager.prototype._waitHandshaken = function(scope, transport, dataId, until, callback) {
	...
	if (!transport.open) {
		scope.log.debug('socketio - WaitHandshaken transport is closed. :' + " dataId = " + dataId);
		callback && callback();
		return;
	}
	var now = new Date().getTime();
	if (now + interval > until) {
		scope.log.warn('socketio - WaitHandshaken timeout. :' + " dataId = " + dataId + " now = " + now + " interval = " + interval + " until = " + until);
		callback && callback();
		return;
	}
	if (!handshaken) {
		scope.log.debug('socketio - WaitHandshaken wating :' + " dataId = " + dataId + " interval = " + interval + " until = " + until);
		setTimeout(scope._waitHandshaken, interval, scope, transport, dataId, until, callback);
	} else {
		scope.log.debug('socketio - WaitHandshaken handshaken :' + " dataId = " + dataId);
		callback && callback();
	}
}; 
